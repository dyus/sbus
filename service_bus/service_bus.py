...
class ServiceBus:
    ...
    async def publish(self, message, routing_key):
        ...
        """
        await self.transport.publish(message, routing_key)
...
class ServiceBusWatchDog:
    """Class for control ServiceBus health and status.
    ...
    def __init__(self, service_bus_factory: typing.Callable, *, backoff: BackOff=None):
        """
        :param service_bus_factory: callable, must return ServiceBus instance
        ...
        self._service_bus: ServiceBus = self._service_bus_factory()

    @property
    def service_bus(self) -> ServiceBus:
    ...
    async def _repair(self):
        ...
        current_service_bus: ServiceBus = self._service_bus
        ...
        try:
            service_bus = self._service_bus_factory()

            await service_bus.connect()
            await asyncio.gather(
                *[service_bus.subscribe(subscriber) for subscriber in subscribers],
                loop=service_bus.loop
            )
